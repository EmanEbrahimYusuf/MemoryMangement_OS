...
void allocateMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	uint32 leastSize=ROUNDUP(size, PAGE_SIZE);
	for(int i=0; i<leastSize/PAGE_SIZE; i++)
	{
...
void freeMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	uint32 PAGENUMBER=size/PAGE_SIZE;
	...
	struct WorkingSetElement *el2=NULL;
	struct WorkingSetElement *el1=NULL;
	va=virtual_address;
	for(int i=0; i<PAGENUMBER;i++)
	{
		LIST_FOREACH(el1,&e->ActiveList)
				{
			if(el1->virtual_address==va)
			{
				LIST_REMOVE(&e->ActiveList,el1);
				LIST_INSERT_HEAD(&e->PageWorkingSetList,el1);
				el1->empty=1;
				unmap_frame(e->env_page_directory,(uint32*)va);

				if(LIST_SIZE(&e->SecondList)!=0)
				{
					el2=LIST_FIRST(&e->SecondList);
					pt_set_page_permissions(e,el2->virtual_address,PERM_PRESENT,0);
					LIST_REMOVE(&e->SecondList,el2);
					LIST_INSERT_TAIL(&e->ActiveList,el2);
				}
			}

				}
		LIST_FOREACH(el1,&e->SecondList)
		{
			if(el1->virtual_address==va)
			{
				LIST_REMOVE(&e->SecondList,el1);
				LIST_INSERT_HEAD(&e->PageWorkingSetList,el1);
				el1->empty=1;
				unmap_frame(e->env_page_directory,(uint32*)va);
			}
	...
	struct Frame_Info *ptrfram=NULL;
	va=virtual_address;
	for(int i=0; i<PAGENUMBER;i++)
	{
		int yy=0;
		get_page_table(e->env_page_directory,(uint32*)va,&ptr_table);
		if(ptr_table!=NULL)
		{
			for(int gg=0;gg<1024;gg++)
			{
				if((ptr_table[gg]&PERM_PRESENT)!=0)
				{
					yy=1;
					break;
				}
			...
			if(yy==0)
			{
				ptrfram=to_frame_info(((e->env_page_directory[PDX(va)])>>12)*PAGE_SIZE);
				ptrfram->references--;

				if((ptrfram->references)==0)
				{
					free_frame(ptrfram);
		...
		va=va+PAGE_SIZE*1024;//new table
	}
}
